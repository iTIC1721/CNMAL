...
namespace CNMAL_Interpreter {
	...
	public partial class Form1 : Form {
		...
		private void TextUpdate() {
			LineCountLabel.Text = "줄 수: " + inputBox.Lines.Length.ToString();
			letterCountLabel.Text = "글자 수: " + inputBox.Text.Length.ToString();
		}
		...
		private void Interpret(string[] text) {
			for (int i = 0; i < text.Length; i++) {
				...
				if (text[i].Contains("겨레의 하나됨과")) {
					Stream stream = Properties.Resources.교가;
